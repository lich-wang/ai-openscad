...
5. 应用从 STL 捕获十四张 PNG：六个正交方向（正视图、后视图、左视图、右视图、俯视图、
   仰视图）和围绕模型的八个等轴测方向。
6. 用户点击 **评审**。
7. 视觉模型检查多角度视图是否满足原始需求，并返回总结、问题列表、置信度和修正提示词。
...
- 对话运行记录包括用户请求、AI 生成/修订、渲染工具开始/进度/完成、评审开始/结果、修正提示词就绪、
  ...
  “渲染工具”，不暴露未解释的内部术语。
- 右侧结果面板只包含十四个具名视图和资产下载控件，不包含编译日志、评审文本、提示词记录或迭代历史。
  视图顺序固定为正视图、后视图、左视图、右视图、俯视图、仰视图、前右上等轴测、前左上等轴测、
  后右上等轴测、后左上等轴测、前右下等轴测、前左下等轴测、后右下等轴测、后左下等轴测。
  桌面端正视图是主视图/最大正交视图，其余十三个角度在下方密集、可滚动网格中保持可见；
  窄屏堆叠布局中十四个视图保持同样阅读顺序和稳定可访问图片名称。
  在 1440 px 桌面视口下，视图网格应至少占右侧面板可见高度的一半。右侧结果面板或视图网格可以内部滚动，
  但整体工作台不能纵向增长到隐藏输入区、工作流操作、高级代码编辑入口或资产下载控件。
  PNG 和 STL 下载控件必须仍能在结果面板内触达，且不能把日志、评审文本或提示词记录混入右侧面板。
...
- 本地视觉回归截图必须明确覆盖十四视图结果面板：正视图仍是主视图/最大视图，十三个辅助视图按稳定顺序位于下方，
  任何内部滚动都限制在结果面板或视图网格内，标签不重叠，没有横向溢出，并且 PNG/STL 下载控件保持可见或可触达。
