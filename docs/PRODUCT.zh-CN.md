...
5. 应用从 STL 捕获正视图、后视图、左视图、右视图、俯视图和等轴测 PNG。
6. 用户点击 **评审**。
...
- 开始再次迭代时，当前轮次的阶段状态会重置：代码生成重新进入处理中，模型渲染等到代码完成后开始，
  模型评审等到新的渲染视图完成后再开始。
- 输入区主按钮在生成、渲染、自动编译修复、评审或导出期间禁用。没有渲染视图时显示“生成”，
  已有视图且没有当前评审时显示“评审”，已评审且没有待接受修订时显示“再次迭代”。
...
- 对话运行记录包括用户请求、AI 生成/修订、渲染工具开始/进度/完成、评审开始/结果、修正提示词就绪、
  ...
  “渲染工具”，不暴露未解释的内部术语。
- 右侧结果面板只包含六个具名视图和资产下载控件，不包含编译日志、评审文本、提示词记录或迭代历史。
  视图顺序固定为正视图、后视图、左视图、右视图、俯视图、等轴测视图。桌面端正视图是主视图/最大视图，
  其余五个角度在下方密集网格中保持可见；窄屏堆叠布局中六个视图保持同样阅读顺序和稳定可访问图片名称。
  在 1440 px 桌面视口下，视图网格应至少占右侧面板可见高度的一半。
- 每个视图 tile 使用对应角度的可见标签和图片 alt 文本：正视图、后视图、左视图、右视图、俯视图、等轴测视图。
  每个 PNG 下载按钮也使用匹配的可访问名称和文件名：
  `ai-openscad-front.png`、`ai-openscad-back.png`、`ai-openscad-left.png`、
  `ai-openscad-right.png`、`ai-openscad-top.png` 和 `ai-openscad-isometric.png`。
...
- 代表性的纹理模型（例如 20cm 高的波浪杯）在草稿评审时应生成低复杂度、可检查的波浪几何，
  ...
  生成的草稿应保持在 45 秒浏览器渲染超时内，并产生可见的正视图、后视图、左视图、右视图、俯视图和等轴测视图。
...
- 本地 E2E 渲染覆盖必须包含用户反馈的完整 20cm 波浪杯 OpenSCAD 文件，包括逐层
  ...
  测试必须断言六个具名视图按顺序出现、每张图片都有非背景模型像素，并且每个 PNG 下载控件有预期可访问名称。
...
- 只有六个视图 key 都有非空图片时才允许视觉评审。发送给 provider 的图片数组必须遵循稳定顺序：
  front、back、left、right、top、isometric。如果捕获失败或少于六张视图可用，应用停留在渲染/错误状态，
  不调用视觉端点。
...
- 渲染证据是有边界的 provider contract，不是本地化 UI 文案。它包含编译状态、
  ...
  视觉评审时必须为 `6`。除非当前模型请求明确需要，否则不会发送 STL 内容、评审图片之外的截图或提示词记录。
- 期望返回 JSON：
  - `summary`
  - `issues`
  - `correctionPrompt`
...
从旧三视图项目导入时，缺失 key 会填为空字符串，直到用户重新渲染。
